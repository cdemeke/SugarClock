...
export class Simulation {
  constructor(now) {
    ...
    this.data = {
      ...
      pomodoro: { durationMs: 25 * 60000, breakDurationMs: 5 * 60000, remainingMs: 25 * 60000, phase: 'work', running: false },
      stopwatch: { elapsedMs: 0, running: false },
      event: { name: 'Birthday', targetEpoch: this.lastNow + 7 * 86400000, remainingMs: 7 * 86400000 },
      ip: '192.168.1.42', providerLabel: false,
      rotation: { enabled: ['glucose', 'time', 'weather', 'pet'], auto: false, intervalMs: 4000 },
      brightness: 100, alertsSnoozedUntil: 0,
    };
  }
  ...
  tick(now = this.now()) {
    ...
    if (p.running) {
      ...
      if (remaining <= 0) {
        ...
        if (remaining <= 0) remaining += Math.floor(-remaining / (p.durationMs + p.breakDurationMs)) * (p.durationMs + p.breakDurationMs);
        while (remaining <= 0) {
          p.phase = p.phase === 'work' ? 'break' : 'work';
          remaining += p.phase === 'work' ? p.durationMs : p.breakDurationMs;
        }
    ...
    this.data.event.remainingMs = Math.max(0, this.data.event.targetEpoch - now);
    if (this.ipUntil && now >= this.ipUntil) {
      this.ipUntil = 0;
      this.lastRotation = now;
    }
    const r = this.data.rotation;
    if (!this.ipUntil && r.auto && r.enabled.length && now - this.lastRotation >= r.intervalMs) {
  ...
  update(section, patch) {
    this.tick();
    if (section === 'ip') { this.data.ip = String(typeof patch === 'string' ? patch : patch.value ?? patch.ip); return; }
    if (!this.data[section] || typeof this.data[section] !== 'object') return;
    if (section === 'glucose' && patch.provider && patch.provider !== this.data.glucose.provider) this.providerUntil = patch.provider.toLowerCase() === 'libre' ? this.lastNow + 1300 : 0;
    Object.assign(this.data[section], patch);
    if (section === 'pomodoro') {
      const p = this.data.pomodoro;
      p.durationMs = Math.max(1000, Number(p.durationMs) || 25 * 60000);
      p.breakDurationMs = Math.max(1000, Number(p.breakDurationMs) || 5 * 60000);
      if ('durationMs' in patch || 'breakDurationMs' in patch) { p.running = false; p.phase = 'work'; p.remainingMs = p.durationMs; }
    }
    if (section === 'rotation') {
      const r = this.data.rotation;
  ...
  preset(status) {
    const presets = { range: { value: 112, trend: 'flat', status: 'range' }, high: { value: 245, trend: 'up', status: 'high' }, low: { value: 58, trend: 'down', status: 'low' } };
    if (presets[status]) this.update('glucose', presets[status]);
  }
  timer(action) {
    this.tick(); const p = this.data.pomodoro;
    if (action === 'reset') { p.remainingMs = p.durationMs; p.phase = 'work'; p.running = false; }
  ...
  stopwatch(action) {
    this.tick(); const s = this.data.stopwatch;
    if (action === 'reset') { s.elapsedMs = 0; s.running = false; }
  ...
  button(name, kind = 'short') {
    ...
    if (name === 'middle') {
      ...
      else if (kind === 'long') this.data.alertsSnoozedUntil = this.lastNow + 30 * 60000;
      else { const levels = [10, 40, 100, 200]; this.data.brightness = levels[(levels.indexOf(this.data.brightness) + 1) % levels.length]; }
    } else if (name === 'left') { if (kind === 'long') this.selectMode('glucose'); else this.next(); }
    else if (name === 'right') {
      if (this.data.mode === 'pomodoro') this.timer(kind === 'long' ? 'reset' : 'toggle');
      else if (this.data.mode === 'stopwatch') this.stopwatch(kind === 'long' ? 'reset' : 'toggle');
      else if (kind === 'long') this.selectMode('glucose');
      else if (this.data.mode === 'pet') { this.data.pet.nap = false; this.data.pet.greetingUntil = this.lastNow + 3000; this.lastRotation = this.lastNow; }
      else { this.advance(-1); this.lastRotation = this.lastNow; }
    }
  }
}
