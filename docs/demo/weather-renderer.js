...
const rgb = (r, g, b, opacity = 255) =>
  "#" +
  [r, g, b]
    .map((v) =>
      Math.floor((v * opacity) / 255)
        .toString(16)
        .padStart(2, "0"),
    )
    .join("");
...
export function renderWeather(
  ...
  const pixel = (x, y, color, icon = true) => {
    if (y >= 0 && y < 8 && x >= (icon ? 0 : 10) && x < (icon ? 8 : 32))
      frame[y * 32 + x] = color;
  };
  const sprite = (rows, width, x, y, color, icon = true) =>
    rows.forEach((bits, dy) => {
      for (let dx = 0; dx < width; dx++)
        if (bits & (1 << (width - dx - 1))) pixel(x + dx, y + dy, color, icon);
    });
  const cloud = (x, y) => {
  ...
  const rounded =
    Math.sign(temperature) * Math.floor(Math.abs(temperature) + 0.5);
  const available =
    Number.isFinite(rounded) && rounded >= -999 && rounded <= 9999;
  ...
  if (!available) sprite([15], 4, 2, 3, rgb(103, 114, 123));
    ...
    switch (visual) {
      ...
      case "partly-cloudy":
        ...
        break;
      case "cloudy":
        cloud(((Math.floor(elapsed / 380) + 7) % 15) - 7, 2);
        break;
      case "cloud-fallback":
        cloud(0, 2);
        break;
      ...
      default: {
        cloud(0, 0);
        const drizzle = visual === "drizzle",
          storm = visual === "storm";
        const lanes = visual === "rain" ? 3 : 2,
          period = drizzle ? 400 : 270;
        for (let i = 0; i < lanes; i++) {
          const step =
            Math.floor(elapsed / period) +
            Math.floor(
              ((elapsed % period) * 5 + i * 12 * period) / (5 * period),
            );
          const y = 4 + (step % 6),
            icy = visual === "sleet" && i === 1;
          const x = storm
            ? i === 0
              ? 0
              : 7
  ...
  const numericWidth = (small) =>
    [...digits].reduce((w, d) => w + (d === "-" || small ? 4 : 5), 0);
