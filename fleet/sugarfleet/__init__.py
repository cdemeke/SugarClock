...
from . import admin, auth, db, device, location_worker
...
def create_app(test_config=None):
    ...
    if not configured_secret and not insecure_mode and not supplied_test_secret:
        raise RuntimeError(
            "FLEET_SECRET_KEY is required unless FLEET_INSECURE_COOKIES=1"
        )
    ...
    app.config.from_mapping(
        ...
        GITHUB_REPOSITORY=os.environ.get(
            "FLEET_GITHUB_REPOSITORY", "cdemeke/SugarClock"
        ),
        GITHUB_API_TOKEN=os.environ.get("FLEET_GITHUB_API_TOKEN", ""),
        PUBLIC_BASE_URL=os.environ.get("FLEET_PUBLIC_BASE_URL", ""),
        DEVICE_CHECKIN_SECONDS=int(
            os.environ.get("FLEET_DEVICE_CHECKIN_SECONDS", "300")
        ),
        IP_GEOLOCATION_ENABLED=os.environ.get("FLEET_IP_GEOLOCATION_ENABLED", "")
        == "1",
        IP_GEOLOCATION_REFRESH_SECONDS=int(
            os.environ.get("FLEET_IP_GEOLOCATION_REFRESH_SECONDS", str(7 * 86400))
        ),
        IP_GEOLOCATION_QUEUE_SIZE=int(
            os.environ.get("FLEET_IP_GEOLOCATION_QUEUE_SIZE", "128")
        ),
        TRUSTED_PROXY_HOPS=int(os.environ.get("FLEET_TRUSTED_PROXY_HOPS", "0")),
        ENROLLMENT_MAX_DEVICES=int(os.environ.get("FLEET_MAX_RECORDS", "10000")),
        ENROLLMENT_GLOBAL_RATE_LIMIT=int(
            os.environ.get("FLEET_ENROLLMENT_GLOBAL_RATE_LIMIT", "100")
        ),
        ENROLLMENT_RATE_LIMIT=int(os.environ.get("FLEET_ENROLLMENT_RATE_LIMIT", "10")),
        ENROLLMENT_RATE_WINDOW_SECONDS=int(
            os.environ.get("FLEET_ENROLLMENT_RATE_WINDOW_SECONDS", "60")
        ),
    ...
    if len(app.config["DEVICE_CREDENTIAL_PEPPER"]) < 32:
        raise ValueError(
            "FLEET_DEVICE_CREDENTIAL_PEPPER must contain at least 32 characters"
        )
    for name in (
        "ENROLLMENT_MAX_DEVICES",
        "ENROLLMENT_GLOBAL_RATE_LIMIT",
    ...
    if (
        app.config["IP_GEOLOCATION_ENABLED"]
        and not trusted_proxy_hops
        and not app.config.get("TESTING")
    ):
        raise ValueError(
            "IP geolocation requires FLEET_TRUSTED_PROXY_HOPS behind the production proxy"
        )
