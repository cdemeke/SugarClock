...
def validate_signed_manifest(manifest, manifest_url, public_keys_dir):
    ...
    if not SEMVER_RE.fullmatch(
        manifest["version"] if isinstance(manifest["version"], str) else ""
    ):
        raise ApiError("invalid_manifest", "manifest version is invalid", 422)
    if not SEMVER_RE.fullmatch(
        manifest["minimum_ota_version"]
        if isinstance(manifest["minimum_ota_version"], str)
        else ""
    ):
        raise ApiError(
            "invalid_manifest", "manifest minimum OTA version is invalid", 422
        )
    ...
    if not SHA256_RE.fullmatch(
        manifest["sha256"] if isinstance(manifest["sha256"], str) else ""
    ):
        raise ApiError("invalid_manifest", "manifest firmware hash is invalid", 422)
    if not immutable_https_url(manifest_url) or not immutable_https_url(
        manifest["firmware_url"]
    ):
        raise ApiError(
            "invalid_manifest", "manifest URLs must be immutable HTTPS URLs", 422
        )
    ...
    if not os.path.isfile(public_key):
        raise ApiError(
            "unknown_manifest_key", "manifest signing key is not trusted", 422
        )
    try:
        signature = base64.b64decode(manifest["signature"], validate=True)
    except (TypeError, ValueError) as error:
        raise ApiError(
            "invalid_manifest_signature", "manifest signature encoding is invalid", 422
        ) from error
    ...
    if result.returncode:
        raise ApiError(
            "invalid_manifest_signature", "manifest signature is invalid", 422
        )
    return manifest
...
def import_manifest(connection, manifest, manifest_url, administrator, public_keys_dir):
    ...
    if existing:
        stored = (
            ...
            existing["hardware"],
            existing["minimum_ota_version"],
        )
        if stored[:4] != identity[:4] or (
            existing["metadata_complete"] and stored != identity
        ):
            raise ApiError(
                "release_immutable_conflict",
                "release version already has different immutable assets",
                409,
            )
        if not existing["metadata_complete"]:
            connection.execute(
                "UPDATE releases SET hardware=?,minimum_ota_version=?,metadata_complete=1 WHERE id=?",
                (manifest["hardware"], manifest["minimum_ota_version"], existing["id"]),
            )
        return existing["id"], False
    cursor = connection.execute(
        "INSERT INTO releases (version, channel, manifest_url, firmware_url, firmware_sha256, "
        "firmware_size, published_at, imported_at, approved_by, hardware, minimum_ota_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            manifest["version"],
            manifest["channel"],
            manifest_url,
...
def _read_json(url, token=None, maximum=512 * 1024):
    ...
    with urllib.request.urlopen(request, timeout=20) as response:
        if response.length is not None and response.length > maximum:
            raise ApiError(
                "github_response_too_large", "GitHub response is too large", 502
            )
    ...
    try:
        ...
        raise ApiError(
            "github_invalid_json", "GitHub returned invalid JSON", 502
        ) from error


def synchronize(
    connection, repository, administrator, public_keys_dir, github_token=None
):
    releases = _read_json(
        f"https://api.github.com/repos/{repository}/releases?per_page=50", github_token
    )
    if not isinstance(releases, list):
        raise ApiError(
            "github_invalid_response", "GitHub releases response is invalid", 502
        )
    imported = 0
    unchanged = 0
    errors = []
        ...
        for asset in release.get("assets", []):
            ...
            try:
                ...
                errors.append(
                    {
                        "asset": name[:128],
                        "code": getattr(error, "code", "github_unavailable"),
                    }
                )
    connection.commit()
