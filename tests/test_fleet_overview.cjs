...
async function boot(initial) {
  ...
  const fleet = {
    ...
    async request(...args) {calls.push(args); if (response instanceof Error) throw response; return response;},
  ...
  await settle();
  return {node, calls, respond(value) {response = value;}, async refresh() {await node('refresh').listeners.click();}};
}
