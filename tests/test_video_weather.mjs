...
const source = readFileSync(
  join(root, "docs/demo/weather-renderer.js"),
  "utf8",
);
const { renderWeather, WEATHER_CONDITIONS, weatherVisualForCondition } =
  await import(
...
test("weather frames match real C++ firmware for every visual, animation phase, and temperature width", () => {
  ...
  try {
    const harness = join(directory, "compare.cpp"),
      executable = join(directory, "compare");
    writeFileSync(
      harness,
  ...
  std::cout << "\\n";
    ...
    const compile = spawnSync(
      ...
      [
        ...
        join(root, "include"),
        harness,
        join(root, "src/weather_render.cpp"),
        "-o",
        executable,
      ],
      { encoding: "utf8" },
    ...
    const cases = [];
    const ids = [
      200, 300, 500, 511, 600, 611, 612, 613, 615, 616, 701, 721, 741, 800, 801,
      802, 803, 804, 781, 711, 999,
    ];
    for (const id of ids)
      for (const elapsed of [
        0, 269, 270, 379, 380, 649, 650, 849, 850, 950, 1600, 3199, 3200, 5700,
        0xffffffff,
      ])
        cases.push([72, 1, id, elapsed]);
    for (const temperature of [
      -999, -100, -9.5, -0.5, 0, 7, 99, 100, 999, 1000, 9999, 10000, -1000,
    ])
    ...
    cases.forEach(([temp, useF, id, elapsed], i) => {
      ...
      assert.equal(
        weatherVisualForCondition(id),
        visuals[visual],
        `mapping ${id}`,
      );
      assert.deepEqual(
        renderWeather(temp, Boolean(useF), id, elapsed).map(rgb565),
        pixels,
        `temp=${temp} unit=${useF} id=${id} elapsed=${elapsed}`,
      );
    });
  } finally {
...
test("all ten conditions are selectable, and animations never affect gutter or temperature", () => {
  ...
  for (const [condition, id] of Object.entries(WEATHER_CONDITIONS)) {
    const frames = [0, 380, 850, 1600, 3200].map((t) =>
      renderWeather(72, true, id, t),
    );
    for (const frame of frames)
      for (let y = 0; y < 8; y++) {
        assert.deepEqual(frame.slice(y * 32 + 8, y * 32 + 10), [null, null]);
        assert.deepEqual(
          frame.slice(y * 32 + 10, y * 32 + 32),
          frames[0].slice(y * 32 + 10, y * 32 + 32),
        );
      }
    if (["sunny", "cloud-fallback"].includes(condition))
      assert(
        frames.every(
