...
test("every mode renders a populated 32×8 matrix of valid LED colors", () => {
  for (const mode of [
    ...
    assert(
      pixels.every((color) => color === null || /^#[a-f0-9]{6}$/i.test(color)),
      mode,
    );
...
for (const kind of [
  ...
  test(`${kind} animates, naps and yields to actual glucose for either alert`, () => {
    ...
    assert.notDeepEqual(
      awake,
      renderFrame({ ...state, pet: { kind, nap: true } }, 0),
      "nap pose",
    );
    for (const [status, value, trend] of [
      ["high", 245, "up"],
      ["low", 58, "down"],
    ]) {
      const alert = {
        ...state,
        glucose: { ...base.glucose, status, value, trend },
      };
      assert.deepEqual(
        renderFrame(alert, 0),
        renderFrame({ ...alert, mode: "glucose" }, 0),
        status,
      );
      assert.match(
        describeFrame(alert, 0),
        new RegExp(`Pet hidden:.*${value}`),
      );
    }
    assert.deepEqual(
      renderFrame(state, 0),
      awake,
      "restoring range restores the pet",
    );
  });
}
...
for (const condition of ["rain", "snow"]) {
  test(`${condition} particles move while the temperature stays stable`, () => {
    ...
    for (let y = 0; y < 8; y++)
      assert.deepEqual(
        a.slice(y * 32 + 9, y * 32 + 32),
        b.slice(y * 32 + 9, y * 32 + 32),
      );
  });
}
...
test("provider identification and work/break phases remain visually distinct", () => {
  ...
  assert.notDeepEqual(
    renderFrame(state, 0),
    renderFrame(
      { ...state, glucose: { ...state.glucose, provider: "libre" } },
      0,
    ),
  );
  ...
  assert.notDeepEqual(
    renderFrame(timer, 0),
    renderFrame(
      { ...timer, pomodoro: { ...timer.pomodoro, phase: "break" } },
      0,
    ),
  );
});
