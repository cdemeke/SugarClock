...
### Reconnection and simpler navigation (build 6)

The home screen opens the last saved clock. My Clocks and Add clock live in a separate sheet, and discovery excludes saved peripheral IDs. Settings use short rows; firmware, diagnostics and advanced controls live under Clock settings. Cached in-memory settings are marked as last loaded while offline; they are never presented as a current glucose reading.

Backgrounding closes the radio session but retains selection and loaded settings. Foreground return reconnects automatically, verifies the bonded device identity and reloads settings before enabling saves. One connection task retries transient connection/read failures up to three times, with cancel and retry controls. Transport waits for Core Bluetooth cancellation before reusing a peripheral, scans during pending reconnection, and shortens post-pairing operation timeouts. A foreground status check every 20 seconds detects a stale link even if iOS still reports it connected. Failed saves are never replayed automatically.

ClockModel now lives in Core so session tests exercise the production coordinator with an injected transport. These cover background return, foreground radio loss, stale links, retry exhaustion, cancellation, identity mismatch, clock switching and lost save acknowledgments. Simulator captures verify presentation; real iPhone/TC001 testing across a five-minute idle interval and network polling remains necessary to establish physical radio reliability. This change does not modify or flash firmware.


### Save receipts and quiet recovery

Settings editors show Saving, Checking save, and Saved on clock beside the save button, with the last confirmation time. Receipts are held per clock in memory and are not erased by reconnect status messages. Settings are read back independently of general clock status, and changed non-secret values are compared before confirmation. Secret values are never retained in a receipt or read back; configured flags require the original save acknowledgment. A lost acknowledgment triggers read-only verification, never an automatic repeat of the write. Later edits are retained if verification finishes after editing resumes.

With previously loaded settings, reconnection uses a small status label and last-sync time. Settings navigation and drafts remain available; controls that send commands still wait for readiness. First-time loading retains its progress indicator. Firmware memory scheduling and radio interruption behavior are unchanged.

### Save confirmation corrections

A matching settings readback confirms a save only when the same response explicitly reports `saved: true`. False or missing durability leaves the operation unconfirmed; a later read-only refresh can finish verification without resending the patch. Secret changes still require the original acknowledgment as well as durable readback.
