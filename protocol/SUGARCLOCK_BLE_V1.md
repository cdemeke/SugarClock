...
New unconfigured clocks with no bonds open a 120-second boot admission window. Configured clocks require holding **only the middle button for at least 3 seconds, then releasing**. A 1–3 second middle hold snoozes alerts on release; a pairing hold does not snooze them or change brightness. Hold only the middle button for **10 seconds, then release** to remove all bonds and reopen admission, preserving NVS application settings and LittleFS certificates. This reset is physical-only: `bonds.reset` returns `physical_action_required`. `status.get` reports bond count and admission-window state. Four bonds maximum; a new phone is refused when full. The custom store handler does not evict another phone's bond.
...
The passkey is rendered as six compact digits for up to 30 seconds. Active buzzer/urgent glucose/urgent notifications take display priority. Retry pairing if an urgent event prevented reading the code. Unauthenticated connections expire at 45 seconds. Authenticated connections expire after 60 seconds without a management read/write or after 10 minutes total. Apps disconnect on backgrounding; temporary inactivity caused by the system pairing sheet must keep the session alive. No background BLE mode is required.
...
`network.pause` indicates that Bluetooth may temporarily disconnect while the clock performs a TLS request. Physical TC001 testing found that keeping NimBLE allocated during Dexcom authentication exhausted heap. The main-loop scheduler protects the first 10 seconds of a connection, pending authentication, fragmented request assembly, queued work and unread response fragments. After those complete it waits for a 1.5-second management-idle gap before suspending BLE for a due network request. A hard 45-second total deferral cap prevents continuous phone traffic from starving autonomous polling; if reached, an unfinished operation can still disconnect and must be treated as unconfirmed. Glucose, scheduled weather, reachability probes and fleet requests share an exclusive reservation; OTA waits for it. BLE resumes after a 1.5-second quiet period following completion so already-due network jobs can share one radio pause, without removing bonds or settings. The boot nonce remains stable during these pauses.

Clients should reconnect to the saved peripheral while foreground, reauthenticate through the platform bond, verify full device identity, and refresh settings/status. The iOS app makes bounded reconnection attempts and retains unsaved editor drafts. Navigation and editing stay available during reconnection; only device commands are disabled. Users can stop/retry reconnection. Settings schema is published only after all pages load and is reloaded after an interrupted first load. It never resends a mutation automatically across sessions. An interrupted save remains unconfirmed until readback; the editor retains its error and draft. These pauses do not stop the glucose/display/alert engines. Active-phone pairing/reconnection and alert-latency stress checks still require physical acceptance.

### Pairing display feedback

After the middle-button admission gesture, the clock shows steady `PAIR` during its bounded admission window. `WAIT` indicates a temporary BLE radio pause for network memory, with admission retained. A six-digit passkey replaces this text only when authenticated pairing starts. Successful authentication closes admission and returns the normal display. OTA, audible/urgent notifications and urgent glucose thresholds take priority. Display layers are composed before one LED submission so glucose and pairing frames do not alternate within a loop. This presentation change does not change the wire protocol.
